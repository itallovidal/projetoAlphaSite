...
function Button({children, variant, ...props}: ButtonProps) {
    ...
    if(variant === "primary"){
        return (
            <ButtonBody {...props} $variant={variant}> {children} <ArrowFatLineRight size={16} weight="fill" /> </ButtonBody>
        );
    }

    if(variant === "secondary"){
        return (
            <ButtonBody $variant={variant}> <ArrowFatLineLeft size={18} weight="fill" /> </ButtonBody>
        );
    }
